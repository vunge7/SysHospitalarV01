...
export default function GraficoTriagem() {
    ...
    useEffect(() => {
        api.get('/linhatriagem/grouped/5')
            .then((res) => {
                const data = res.data;
                ...
                // SATURAÇÃO OXIGÊNIO
                setSpo2Trend(
                    (data.SATURACAO_OXIGENIO || []).map((item) => ({
                        time: item.dataCriacao,
                        value: Number(item.valor),
                    }))
                );
                ...
                setRespTrend(
                    (data.FREQUENCIA_RESPIRATORIA || []).map((item) => ({
                        time: item.dataCriacao,
                        value: Number(item.valor),
                    }))
                );
                ...
                // TEMPERATURA
                setTempTrend(
                    (data.TEMPERATURA || []).map((item) => ({
                        time: item.dataCriacao,
                        value: Number(item.valor),
                    }))
                );

                // PRESSÃO ARTERIAL (ex: valor = "120/80")
                setMmhgTrend(
                    (data.PRESSAO_ARTERIAL || []).map((item) => {
                        const [sistolica, diastolica] = (item.valor || '0/0')
                            .split('/')
                            .map(Number);
                        return {
                            time: item.dataCriacao,
            ...
            <div className="monitor-info">
                <div className="vital">
                    ...
                    <span className="value">
                        {pulseTrend[pulseTrend.length - 1]?.value}
                    </span>
                ...
                <div className="vital">
                    ...
                    <span className="value">
                        {spo2Trend[spo2Trend.length - 1]?.value}
                    </span>
                </div>
                <div className="vital">
                    <span className="icon">🫁</span>
                    ...
                    <span className="value">
                        {respTrend[respTrend.length - 1]?.value}
                    </span>
                ...
                <div className="vital">
                    ...
                    <span className="value">
                        {tempTrend[tempTrend.length - 1]?.value}
                    </span>
