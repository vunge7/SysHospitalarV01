...
function Exame({ exames, medicos, setExames, fetchAllData, createExame, updateExame, deleteExame }) {
  ...
  // CORREÇÃO: Filtra por productGroup === "Exames"
  const fetchProdutosExame = async () => {
    try {
      ...
      const exames = todos
        .filter(p => p.productGroup === 'Exames')           // Apenas produtos do grupo "Exames"
        .filter(p => !p.produtoPaiId);                      // Apenas raízes (não filhos)

      setProdutosExame(exames);
    } catch (error) {
      console.error('Erro ao carregar produtos de exame:', error);
      setProdutosExame([]);
      toast.error('Falha ao carregar exames.', { autoClose: 2000 });
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space>
          <Button icon={<EyeOutlined />} onClick={() => showDetailsModal(record)}>Detalhes</Button>
          ...
          <Popconfirm
            ...
            cancelText="Não"
          >
            <Button icon={<DeleteOutlined />} danger>Excluir</Button>
          </Popconfirm>
        </Space>
      ),
    },
        ...
        <Table
          columns={[
            ...
            },
            {
              ...
              dataIndex: 'status',
              key: 'status',
              render: s => (s === true || s === '1' || s === 1 || s === 'true' || s === 'ATIVO') ? 'Ativo' : 'Inativo'
            },
            {
              title: 'Ações',
              key: 'acoes',
        ...
        {produtoSelecionado && (
          <div className="modal-body">
            ...
            {produtoSelecionado.imagem && (
              <img
                src={produtoSelecionado.imagem.startsWith('http') ? produtoSelecionado.imagem : `/produto/imagens/${produtoSelecionado.imagem}`}
                alt="Produto"
