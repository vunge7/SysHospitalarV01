...
const GraficoAgenda = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const dadosGrafico = processarDados(
                    linhasAgenda,
                    pacientes,
                    funcionarios,
                    pessoas
    ...
    const processarDados = (linhasAgenda, pacientes, funcionarios, pessoas) => {
        ...
        const medicosUnicos = [
            ...new Set(
                linhasAgenda.map(
                    (la) => medicosMap[la.funcionarioId] || 'Desconhecido'
                )
            ),
        ];
        ...
        const anosUnicos = [
            ...new Set(
                linhasAgenda
                    ...
                    .map((linha) =>
                        new Date(linha.dataRealizacao).getFullYear()
                    )
            ),
        ...
        const consultasAgrupadas = linhasFiltradas.reduce((acc, linha) => {
            const nomeMedico =
                medicosMap[linha.funcionarioId] || 'Desconhecido';
            const nomePaciente =
                pacientesMap[linha.pacienteId] || 'Desconhecido';
    ...
    return (
        <div className="consulta-grafico-container">
            ...
            {loading ? (
                ...
                <p className="consulta-grafico-no-data">
            ...
            {selectedConsulta && (
                <div className="consulta-grafico-details">
                    <h3 className="consulta-grafico-details-title">
